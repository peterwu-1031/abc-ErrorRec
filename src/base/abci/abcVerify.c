...
static void  Abc_NtkVerifyErrorRecPatch( Abc_Ntk_t * pNtk1, Abc_Ntk_t * pNtk2, int * pModel, sat_solver ** pSat );
...
/**Function*************************************************************
  ...
  Synopsis    [Verifies sequential equivalence by fraiging followed by SAT and generate all error patterns.]
...
void Abc_NtkCecAll( Abc_Ntk_t * pNtk1, Abc_Ntk_t * pNtk2, int nSeconds, int fVerbose )
{
    abctime clk = Abc_Clock();
    Prove_Params_t Params, * pParams = &Params;
    Abc_Ntk_t * pMiter, * pTemp;
    ...
    mkdir("cec_output", S_IRWXU);
    // Generate files for error patterns.
    f = fopen("cec_output/filename.txt", "w");
    fin = fopen("cec_output/inputID.txt", "w");
    for ( i = 0; i < Abc_NtkCoNum(pNtk1); i++ )
    {
        sprintf(filename, "cec_output/%s_error1>0.txt", Abc_ObjName(Abc_NtkCo(pNtk1,i)));
        temp = fopen(filename, "w");
        fclose(temp);
        sprintf(filename, "%s_error1>0.txt", Abc_ObjName(Abc_NtkCo(pNtk1,i)));
        fprintf(f, "%s\n", filename);
        sprintf(filename, "cec_output/%s_error0>1.txt", Abc_ObjName(Abc_NtkCo(pNtk1,i)));
        temp = fopen(filename, "w");
        fclose(temp);
        sprintf(filename, "%s_error0>1.txt", Abc_ObjName(Abc_NtkCo(pNtk1,i)));
        fprintf(f, "%s\n", filename);
    }
    fclose(f);
...
// Revised version of Abc_NtkCecFraig
void Abc_NtkCecPatch( Abc_Ntk_t * pNtk1, Abc_Ntk_t * pNtk2, int nSeconds, int fVerbose )
{
    abctime clk = Abc_Clock();
    Abc_Ntk_t * pExdc = NULL;
    Abc_Ntk_t * pMiter, * pTemp;
    ...
    Vec_Ptr_t * vNodes;
    Abc_Obj_t * pNode;
    char * PiName[Abc_NtkCiNum(pNtk1)];
    int times = 1, not_done = 1, first_iter = 1, num_pi, num_mux = 0, i, k;
    ...
    int max_rand_err = 3, RandErr = 0, errors = 0;

    // make a directory for cec output files
    mkdir("cec_patch", S_IRWXU);
    // Generate files for error patterns.
    f = fopen("cec_patch/patch.v", "w");
    ...
    for ( k = 0; k < Abc_NtkCoNum(pNtk1); k++ )
    {
        pSat = NULL;
        not_done = 1;
        num_pi = 0;
        ...
        while ( times < max_iter )
        {
            ...
            //         {
            //             printf("Iteration %d:\n", times);
            //             Abc_NtkVerifyErrorRecPatch( pNtk1, pNtk2, pMiter->pModel, &pSat );
            //             times++;
            //             errors = 0;
            //             RandErr = 0;
            ...
            if ( RetValue == 1 )
            {
                printf( "%s is equivalent after structural hashing.  ", PoName );
                Abc_NtkDelete( pMiter );
                Abc_PrintTime( 1, "Time", Abc_Clock() - clk );
                break;
            }
            ...
            else if ( RetValue == 0 && not_done == 1 )
            {
                ...
                if ( pSimInfo[0] != 1 )
                {
                    ...
                    printf( "%s is NOT EQUIVALENT.  ", PoName );
                ABC_FREE( pSimInfo );
            }
            else
            {
                printf( "%s is equivalent.  ", PoName );
                Abc_PrintTime( 1, "Time", Abc_Clock() - clk );
                break;
            }
            Abc_PrintTime( 1, "Time", Abc_Clock() - clk );
            if ( pMiter->pModel )
            {
                ...
                // write the pattern to the blif file and add the corresponding clause to SAT solver
                if ( first_iter == 1 )
                { 
                    first_iter = 0;
                ...
                fprintf(temp, ")");
                printf("\n");
                pSat->temp = sat_solver_addclause(pSat, Lits, Lits + num_pi);
            }
            not_done = pSat->temp;
            times += 1;
        }
...
/**Function*************************************************************
  ...
  Synopsis    [Report errors and do error rectifications.]
...
void Abc_NtkVerifyErrorRec( Abc_Ntk_t * pNtk1, Abc_Ntk_t * pNtk2, int * pModel, sat_solver ** pSat )
{
    ...
    for ( j=0; j<nErrors; j++ )
    {
        ...
        if ( pValues1[Node[j]] ) sprintf(filename, "cec_output/%s_error1>0.txt", Abc_ObjName(Abc_NtkCo(pNtk1,Node[j])));
        else sprintf(filename, "cec_output/%s_error0>1.txt", Abc_ObjName(Abc_NtkCo(pNtk1,Node[j])));
        f = fopen(filename, "a");
...
// Revised version of Abc_NtkVerifyReportError
void Abc_NtkVerifyErrorRecPatch( Abc_Ntk_t * pNtk1, Abc_Ntk_t * pNtk2, int * pModel, sat_solver ** pSat )
{
    Vec_Ptr_t * vNodes;
    Abc_Obj_t * pNode;
    int * pValues1, * pValues2;
